...
@app.route('/atualizacao-status-envios')
def index():
    if  str(request.authorization['username']) == f"{os.getenv('USER_API_KEY')}" and str(request.authorization['password']) == f"{os.getenv('PASS')}":
        data_inf = []
